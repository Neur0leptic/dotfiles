...
update_nvidia() {
	grep -q 'VIDEO_CARDS="nvidia"' /etc/portage/make.conf || { echo "Not using Nvidia GPU."; return; }
        [[ "${KERNEL_IS_UPDATED}" == "0" ]] && {
...
handle_shutdown() {
        ...
        delay_shutdown="$(echo -e "No\nYes" | bemenu -l "2" -p "Do you want to delay shutdown?")"

        [[ "${delay_shutdown}" == "Yes" ]] && {
                while true; do
                        delay_amount="$(echo "" | bemenu -l "0" -p "Enter delay amount in minutes:")"
                        ...
                        delay_shutdown="$(echo -e "No\nYes" | bemenu -l "2" -p "Do you want to delay shutdown again?")"
                        [[ "${delay_shutdown}" == "No" ]] && {
                                openrc-shutdown -p "now"
                                break
                        }
