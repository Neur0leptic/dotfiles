...
[ -s "${HOME}/.config/.mymlocate.db" ] || {
        ...
        disk_path="$(echo "" | bemenu -l "0" -p "Enter the disk path (e.g '/mnt/harddisk'): ")"
...
chosen_file="$(echo "${video_files}" | sed 's|.*/||; s/\.[^.]*$//' | bemenu -p "Select Video")"
...
# Remove the randomly selected video
# CHOICE="$(printf "Yes\nNo" | bemenu -l "2")"
